...
It does so by \emph{s}martly \emph{match}ing agents and items which make up the parts of a bipartite graph.
The second one, \emph{\RepReMatch}, computes a \(\paren[\big]{2n (\log_2 n + 3)}\)-approximative allocation when the valuation functions are submodular.
...
In \cref{sec:hardness}, we give an analysis of the hardness of the submodular \NSW.
