\begin{algorithm}
	...
	\KwOut{
		\(\sfrac{1}{2n}\)-approximation \(\alloc = (\alloc[1], \dots, \alloc[n])\) of optimal allocation
	}

	\(\alloc[i] \gets \emptyset \quad\forall i \in \agents\)\;
	\(\remvalue{i} \gets \valuations[i][\goodsint{i}{2n+1:m}] \quad\forall i \in \agents\)\;
	\(\weights \gets \{\,  \weight[i] \cdot \log\bigl( \valuations[i][j] + \frac{\remvalue{i}}{n} \bigr) \given[\bigm] \forall i \in \agents \forall j \in \goods \,\}\)\tcp*{graph weights}
	\(\bipartitegraph \gets (\agents, \goods, \weights)\)\tcp*{bipartite graph}
	...
	\While{\(\goodsrem \neq \emptyset\)}{
		\(\weights \gets \{\,  \weight[i] \cdot \log\bigl( \valuations[i][j] + \valuations[i][\alloc[i]] \bigr) \given[\bigm] \forall i \in \agents \forall j \in \goodsrem \,\}\)\;
