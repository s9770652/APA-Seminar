...
For convenience, we order the items in any bundle \(\{\asgd{1}, \asgd{2}, \dots \}\) by the order in which they were assigned to agent \(i\), so that item \(\asgd{t}\) is assigned according to the \(t\)th matching.
...
\begin{proof}
	...
	Thus, assigning all agents~\(i\) their respective most highly valued item \(\asgd*{1}\) from an optimal bundle~\(\alloc*\) yields the even lower bound
	\begin{equation}
