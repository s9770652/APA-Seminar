...
\begin{algorithm*}
	...
	\(\weights \gets \{\,  \weight{i} \cdot \log\bigl( \valuations[i][j] + \frac{\remvalue{i}}{n} \bigr) \given[\bigm] i \in \agents, j \in \goods \,\}\)\tct*{edge weights}
...
\begin{figure*}
	...
	\caption{%
		...
		The approximation factor \(\alpha \approx \sqrt{m/2}\) is therefore dependant on the number \(m\) of items.
	}
\end{figure*}