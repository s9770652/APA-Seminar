...
For convenience, we order the items in the final allocation \(\alloc = \{\asgd{1}, \dots, \asgd{\alloclen}\}\) of agent \(i\) by the order in which they were assigned, so that item \(\asgd{t}\) is assigned according to the \(t\)-th matching.
...
\begin{lemma}
	...
	For each agent \(i \in \agents\) and her final allocation \(\alloc = \{\asgd{1}, \dots, \asgd{\alloclen}\}\), the item \(\asgd{t}\) is worth at least as much as the overall \(tn\)-th most highly valued item \(\goodsordered{tn}\), \ie{} \(\valuations[\asgd{t}] \geq \valuations[\goodsordered{tn}]\), for all \(t = 1, \dots, \alloclen\).
\end{lemma}
\begin{proof}
...
\begin{lemma}
	...
	For each agent \(i \in \agents\) and her final allocation \(\alloc = \{\asgd{1}, \dots, \asgd{\alloclen}\}\), it holds \(\valuations[ \asgd{2}, \dots, \asgd{\alloclen} ] \geq \remvalue/n\), where \(n\) is the number of items and \(\remvalue = \valuations[ \goodsordered{2n+1}, \dots, \goodsordered{m} ]\).
\end{lemma}
\begin{proof}
	...
	Further, it holds \(\alloclen n + n \geq m \) since each agent receives items for at least \(\floor{\frac{m}{n}} \geq \frac{m}{n} - 1\) rounds.\todo{unless there are zero valuations}
...
This allows us to calculate an approximation factor for \SMatch.
\begin{theorem}
	\label{th:smatch}
