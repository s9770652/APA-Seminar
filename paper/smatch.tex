...
For convenience, we order the items of any set \(\genericset = \{\itordinal \genericitem[1], \genericitem[2], \dots \}\) in descreasing order of valuation, so that it holds \(\valuations[\itordinal\genericitem[k]] \ge \valuations[\itordinal\genericitem[k']]\) for all \(k < k'\).
