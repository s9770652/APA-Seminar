...
\begin{algorithm*}[p]
	...
	\(\weights \gets \{\,  \weight \cdot \log\bigl( \valuations[j] + \frac{\remvalue}{n} \bigr) \given[\bigm] i \in \agents, j \in \goods \,\}\)\tct*{edge weights}
	...
	\While{\(\goodsrem \neq \emptyset\)}{
		\(\weights \gets \{\,  \weight \cdot \log\bigl( \valuations[j] + \valuations[\alloc] \bigr) \given[\bigm] i \in \agents, j \in \goodsrem \,\}\)\;
