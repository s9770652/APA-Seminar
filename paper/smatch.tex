...
As \cref{fig:allocation} demonstrates, this leads to an algorithm with an approximation factor dependent on the number \(m\) of items.
...
\begin{figure*}
	...
	\caption{%
		...
		Even if all remaining items were going to be assigned to agent \(\agents_{1}\), the NSW would never surpass \(\sqrt{(2m + \epsilon - 1) \cdot 1} < \sqrt{2m}\).
		The approximation factor therefore is at least about \(\sqrt{m/2}\) and, thus, depends on the number of items.
	}
...
\end{equation}
Note that set \(\genericset\) has less than \(2n\) elements only if there are less than \(2n\) items in total.
...
\begin{algorithm*}[t]
	...
	\While{\(\goodsrem \neq \emptyset\)}{
		\(\weights \gets \braces[\big]{\, \paren[\big]{ i, j, \weight \log\paren{ \valuations[j] + \valuations[\alloc] } } \given[\bigm] i \in \agents, j \in \goodsrem \,}\)\;
...
For convenience, we order the items in a bundle \(\alloc = \{\asgd{1}, \asgd{2}, \dots \}\) by the order in which they were assigned to agent \(i\), so that item \(\asgd{t}\) is assigned according to the \(t\)th matching.
...
\begin{proof}
	...
	This yields\todo[info]{\(\mathbfit{i}\): I do not get the reason for the extra step in the original paper.}
	\begin{equation}
...
\begin{remark}
	...
	However, \SMatch{} does not produce allocations which are \emph{Pareto-optimal (\PO)}, which is another popular fairness property.
