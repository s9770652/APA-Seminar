\begin{algorithm*}
	...
	\For{\(t = 0, \dots, \ceil{\log n}-1\)}{
		\If{\(\goodsrem \neq \emptyset\)}{
			\(\weights \gets \{\,  \weight \cdot \log\bigl( \valuations[j] \bigr) \given[\bigm] i \in \agents, j \in \goods \,\}\)\;
	...
	\While{\(\goodsrem \neq \emptyset\)}{
		\(\weights \gets \{\,  \weight \cdot \log\bigl( \valuations[ \alloc[\phaseii] \cup \{j\} ] \bigr) \given[\bigm] i \in \agents, j \in \goods \,\}\)\;
		\(\bipartitegraph \gets (\agents, \goods, \weights)\)\;
		\(\matching \gets \maxweightmatching(\bipartitegraph)\)\;
