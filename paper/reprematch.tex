...
%	Now choose some matching \(\matching[t]\) which maximises the number of agents matched with one of their overly good items, and denote by \(\unluckyagents{t}\) the set of agents who are \emph{not} matched with one of their overly good items.
%	We prove by induction on the number \(t\) of rounds that it holds \(\abs{\unluckyagents{t}} \leq n/2^t\).
...
%	\end{caseintext}
	...
	Imagine that for some \(t\) only the items assigned in the first~\(t\) rounds of phase~\phasei{} were released in the beginning of phase~\phaseiii.
	...
	In the induction step \(t \to t+1\), there is, by \cref{enum:matching:agent}, at least one unassigned item in each set \(\overlygoodset[i']\) for all agents \(i' \in \unluckyagents{t}\) at the start of round \(t+1\).
...
\begin{theorem}
	\label{th:reprematch}
	\RepReMatch{} has an approximation factor of \(2n (\log_2 n + 2)\).
\end{theorem}
\begin{proof}
	...
	\RepReMatch{} maximises the logarithmic Nash social welfare, so we get
	\begin{equation}
	...
	Phase \phasei{} runs for at most \(\ceil{\log_2 n}+1\) rounds and in each iteration at most \(n\) items are assigned.
	Therefore, at most \(n (\log_2 n + 2)\) optimal items are assigned in that phase, \ie, \(\abs{ \alloc* \setminus \attopt{0} } \leq n (\log_2 n + 2)\).
	Further, remember that in each round of phase \phaseii, at most \(n\) optimal items are assigned to other agents, so it holds \(n \geq \lostsetlen{1} = \abs{\lostset{1}}\).
	...
	\begin{equation}
		...
		\geq \valuations[\asgd*{1}]
		\geq \frac{\valuations[ (\alloc* \setminus \attopt{0}) \cup \lostset{1} ]}{n(\log_2 n + 3)}
	\end{equation}
	\todo[inline]{Was ist das für eine Gleichung?}
	\Cref{lem:lower_bound_later_items} and the monotonicity of valuations yield
	\begin{equation}
	...
	\end{equation}
	The mean of \crefrange{eq:overly_good_item_lower_bound}{eq:assigned_items_lower_bound} and the monotonicity of valuations gives the concise lower bound
	\begin{align}
		\valuations[ \overlygooditem, \asgd{1}, \dots, \asgd{\alloclen[\phaseii]} ]
		&\geq \frac{1}{2} \paren*{ \frac{\valuations[ (\alloc* \setminus \attopt{0}) \cup \lostset{1} ]}{n(\log_2 n + 3)} + \frac{\valuations[ \attopt{0} \setminus \lostset{1} ]}{n(\log_2 n + 3)} } \\
		&\geq \frac{1}{2} \cdot \frac{ \valuations[ \paren[\big]{(\alloc* \setminus \attopt{0}) \cup \lostset{1}} \cup \paren[\big]{\attopt{0} \setminus \lostset{1}} ] }{n(\log_2 n + 3)} \\
...
\begin{algorithm*}[p]
	...
	\KwOut{%
		\(\frac{1}{2n(\log_2 n + 2)}\)-approximation \(\alloc[\phaseiii][] = (\alloc[\phaseiii][1], \dots, \alloc[\phaseiii][n])\) of an optimal allocation
	}
	\phaseisep
