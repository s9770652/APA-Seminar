...
The algorithm \SMatch{} estimates the valuation of lowest-value items by determining the set of highest-value items and then valuing the remaining items.
This approach does not work for submodular valuations in general because taking the set of highest-value items away may not leave a set of lowest-value items.
...
As a consequence, the algorithm \RepReMatch, described in \cref{alg:reprematch}, relies on an approach with three phases and achieves an approximation factor of \(2n(\log_2 n + 3)\) (cf.\@ \cref{th:reprematch}).
In phase~\phasei, a sufficiently big set of high-value items is determined through repeated matchings.
This phase serves only to determine this set, so matched items are temporarily put away but not assigned.
The edge weights reflect this by taking the valuations of only single items into account.

In phase~\phaseii, the remaining items are assigned normally through repeated matchings.
Each edge weight is, thence, the weighted logarithm of the valuation of the set of both the respective item and the set of items assigned so far.

In phase~\phaseiii, the high-value items put away in phase~\phasei{} are brought back.
...
Again each edge weight is the weighted logarithm of the valuation of the set of both the respective item and the set of items assigned so far.
The remaining items are assigned arbitrarily.

We start with analysing phase~\phaseii{} as it is the first phase with final assignments.
...
\begin{corollary}
	...
	\begin{equation*}
		...
		\end{multlined}
	\end{equation*}
	where \(\attoptlen{r} \coloneq \abs{\attopt{0}}\) denotes the number of optimal and attainable items in \(r\) round.
\end{corollary}
\begin{proof}
...
\begin{proof}
	...
	Furthermore, after \(\alloclen[\phaseii]\) rounds, the number \(\attoptlen{\alloclen[\phaseii]}\) of optimal and attainable items is at most \(n-1 \leq n\) else agent \(i\) would have been assigned yet another item.
...
Therefor we introduce the concept of \emph{overly good} items, that is items which some agent \(i\) values at least as high as her most valuable optimal item~\(\asgd*{1}\).
...
\end{lemma}
\begin{proof}
	If in phase~\phasei{} all items are put aside, \ie, \(\goodsrem = \emptyset\), then the optimal item \(\asgd*{1}\) of each agent \(i\) is brought back in phase~\phaseiii{} and can be assigned to her;
	the lemma is proven immediately.
	If not, imagine that for some \(t\) only the items assigned in the first~\(t\) rounds of phase~\phasei{} were brought back.
	Denote the set of brought-back items by \(\goodsreleased{t}\).
	Now choose some matching \(\matching[t]\) with the following properties:
	\begin{enumerate}
		\item
		\label[property]{enum:matching:agent}
		If for an agent \(i\) all overly good items were amongst the released items, \ie, \(\overlygoodset \subset \goodsreleased{t}\), she gets matched with an overly good item \(j \in \overlygoodset\).
	...
	\end{enumerate}
	\Cref{enum:matching:agent} is always satisfiable as each set \(\overlygoodset\) uniquely contains the item \(\asgd*{1}\), which can be matched with agent \(i\).
	...
	Analogously to the base case, for at least half of those agents \(i'\) these unassigned items will be assigned to them or someone else.
...
\begin{proof}
	...
	\end{equation}
	is a lower bound on the \NSW{} after the first matching in phase~\phaseiii, whereby \(\{ \asgd{1}, \dots, \asgd{\alloclen[\phaseii]} \}\) is the bundle of agent \(i\) assigned in phase~\phaseii.
	...
	Phase~\phasei{} runs for at most \(\ceil{\log_2 n}+1\)\todo[info]{Error in paper‽ see also \cref{lem:overly_good_matching}} rounds, and at most \(n\) items are assigned in each iteration.
	Therefore, at most \(n (\log_2 n + 2)\) optimal items are assigned in that phase, \ie, \(\abs{ \alloc* \setminus \attopt{0} } \leq n (\log_2 n + 2)\).
	Furthermore, in each round of phase~\phaseii, at most \(n\) optimal items are assigned to other agents, so that it holds \(n \geq \lostsetlen{1} = \abs{\lostset{1}}\).
	...
	\end{equation}
	as lower bound on the individual valuations of assigned bundles.
	...
	\begin{equation}
		\log \NSW(\alloc[\phaseiii][])
		\geq \frac{1}{\sum_{i \in \agents} \weight} \cdot \sum_{i \in \agents} \weight \log \frac{ \valuations[\alloc*] }{2n(\log_2 n + 3)}
		= \log \paren[\bigg]{\frac{\NSW(\alloc*[][])}{2n(\log_2 n + 3)}}
	\end{equation}
\end{proof}

\todo[inline]{released → brought back; Notation ändern}
\todo[inline]{phase aus Algo entfernen}