...
Note that the term \emph{round} is used to refer to the iterations of the \texttt{while}-loop in phase \phaseii{} and later also for the \texttt{for}-loop in phase \phasei.
...
\begin{proof}
	...
	If not, imagine for some \(t\) that only the items assigned in the first~\(t\) rounds of phase~\phasei{} were brought back.
...
\todo[inline]{brought back → released}