...
First, we bound\todo[info]{\(\mathbfit{i}\): Would \enquote{to bound} (strongly) imply giving an upper bound as well?} the valuation of an agent \(i\) for her optimal items which still may be assigned to her in phase \phaseii*.
\begin{lemma}
	\label{lem:induction}
...
\begin{proof}
	...
	For a more detailed formula simplification, we refer to Garg, Kulkarni and Kulkarni~\cite[14]{APNSWuSVþUM}.\todo[info]{\(\mathbfit{i}\): I hope the shortcut is allowed. Might write out it if rest of document not too long.}
\end{proof}

The lemma can be used to bound the marginal valuation of the items assigned in each round \(r\).
\begin{corollary}
...
\begin{proof}
	...
	Together with \cref{cor:lower_bound_single_item}, this proves the lemma:\todo[info]{\(\mathbfit{i}\): swapping the summands' positions reduces the number of lines used}
	\begin{align}
...
\begin{proof}
	\todo[info, inline]{%
		...
		It could be that I understand the source of confusion but I cannot guarantee this yet.
		I guess I'll have to inform you via email next week...
	}
