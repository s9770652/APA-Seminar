...
For this reason the algorithm \RepReMatch, described in \cref{alg:reprematch}, relies on an approach with three phases and achieves an approximation factor of \(2n(\log_2 n + 3)\) (cf.\@ \cref{th:reprematch}).
...
To this end, we look at the set \(\lostset{r}\) of \emph{lost items}, that is the optimal items of an agent \(i\) which are assigned to other agents \(i' \neq i\) in round \(r\),
and the set \(\attopt{r}\) of \emph{optimal and attainable items}, that is the optimal items of agent \(i\) minus all lost items in \(\lostset{1} \cup \dots \cup \lostset{r}\) and minus the items assigned to her up until round \(r-1\).
We denote their sizes by \(\lostsetlen{r}\ \coloneq \abs{\lostset{r}}\) and \(\attoptlen{r} \coloneq \abs{\attopt{r}}\), respectively.
First, we give a lower bound on the valuation of agent \(i\) for her optimal and attainable items.
\begin{lemma}
	\label{lem:induction}
...
\begin{corollary}
	...
	\begin{equation*}
		...
		\end{multlined}
	\end{equation*}
	where \(\attoptlen{r}\) denotes the number of optimal and attainable items in round \(r\).
\end{corollary}
\begin{proof}
...
\todo[inline]{more comments in algo}