\section{\texorpdfstring{\RepReMatch}{RepReMatch}}
...
\begin{algorithm*}[!b]
	...
	\KwOut{%
		\(\frac{1}{2n(\log_2 n + 3)}\)-approximation \(\alloc[\phaseiii][] = (\alloc[\phaseiii])_{i \in \agents}\) of an optimal allocation
	}
	\phaseisep
...
\begin{proof}
	...
	\end{align}
	where the second inequality can be shown inductively with the definition of submodularity\todo{Do that or, alternatively, find a paper showing the other def. [IRfCAwSUF]}, and the third inequality is due all \(\lostsetlen{2}\) items \(j\) in set \(\lostset{2}\) not being assigned in round \(1\) although attainable, implying \(\valuations[j] \le \valuations[\asgd{1}]\).
	...
	\begin{caseintext}{1}{\(\asgd{r} \in \attopt{r}\)}
		Again we exploit the submodularity of the valuation functions \todo{ditto} to obtain a lower bound on the marginal valuation of \(\attopt{r+1}\).
		\begin{align}
...
%	Of those, \(\lostsetlen{l}\) many are assigned to other agents in each round \(l \le r\), and also some items \(\asgd{l}\) assigned to agent \(i\) may be optimal, whence an upper bound of \(\attoptlen{0} - \sum_{l=1}^{r} \lostsetlen{l}\) on the number \(\attoptlen{r}\) of items in the set \(\attopt{r}\).\todo{Possibly this whole estimation can be omitted as we ditch it in the following lemma.}
	...
	The valuation functions are monotonic, \ie, \(\valuations[\genericset[1]] \le \valuations[\genericset[2]]\) for all item sets \(\genericset[1] \subset \genericset[2] \subset \goods\).\todo{change proof when intro is finished}
...
%		\ie, \(\overlygoodset \subset \goodsreleased{t}\),
	...
	\end{enumerate}
	\Cref{enum:matching:agent} is always satisfiable as each set \(\overlygoodset\) is the only one to contain the item \(\asgd*{1}\), which can be matched with agent \(i\).\todo{wrong! the union contains it though}
...
\begin{theorem}
	\label{th:reprematch}
	\RepReMatch{} is \(\paren[\big]{2n (\log_2 n + 3)}\)-approximative.
\end{theorem}
\begin{proof}
