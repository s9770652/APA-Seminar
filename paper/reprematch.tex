...
%	Denote the set of released items by \(\goodsreleased{t}\).
	...
	\begin{enumerate}
		...
		If all outstanding items of an agent \(i\) were amongst the released items,
		\ie, \(\overlygoodset \subset \bigcup_{i' \in \agents} \alloc[\phasei][i']\),
		she gets matched with an outstanding item \(j \in \overlygoodset\).
