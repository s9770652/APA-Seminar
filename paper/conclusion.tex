...
\begin{itemize}
	...
	\Citeauthor{sublin_approx_algo_for_nsw_with_xos_valuations}~\cite{sublin_approx_algo_for_nsw_with_xos_valuations} used both \RepReMatch{} and the discrete moving-knife method to get an \(\bigo(n^{53/54})\)-approximation algorithm.
...
%	\item
%	\CASC{} (also known as \SPLC) functions encompass additive functions and can also model diminishing returns.
...
%	\Citeauthor{APNSWuSVþUM} emphatically mentioned the lack of research into the \OXS{} \NSW.
	\Citeauthor{approximating_nsw_under_rado_valuations}~\cite{approximating_nsw_under_rado_valuations} developed an algorithm which is divided into five phases and shares ideas with \RepReMatch.
	%
...
%Particularly \RepReMatch{} with no fairness guarantee is unsatisfactory.
